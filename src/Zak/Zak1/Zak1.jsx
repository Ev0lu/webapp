            ...
            <div className={s.greetings_wrapper}>
                ...
                <div className={s.date_picker}>
                    <div className={s.date_flex}>
                        ...
                        {errorFields.err && <span className={s.error_message}>Вы должны быть старше 18 лет и не младше 100 лет</span>}
                    </div>
                    <div className={`${s.icon} ${errorFields.selectedDate && s.open}`} onClick={toggleCalendar}>
                        {showCalendar ? <img src={props.colorB === 'light' ? lightplus : plus} alt="Toggle calendar" /> : <img src={props.colorB === 'dark' ? minus : lightminus} alt="Toggle calendar" />}
                    </div>
                    {showCalendar && (
                ...
                <Link to={(selectedDate !== null && !err) ? "/zak2_reg" : "/zak1_reg"}>
                    <button onClick={validateFields} className={`${s.greetings_btn} ${props.colorB === 'light' ? s.lightMode : s.darkMode}`} disabled={(selectedDate === null || err)}>
                        Далее
                    </button>
                </Link>
            </div>
